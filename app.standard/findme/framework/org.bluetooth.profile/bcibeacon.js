...
cordova.define("org.bcsphere.bcibeacon", function(require, exports, module) {
/*
...
*/
    ...
    var IBeaconClass = cordova.require("org.bcsphere.ibeacon");
	...
	var IBeaconManager = BC.IBeaconManager = BC.Plugin.extend({
		...
		pluginInitialize : function(){
			...
			if(API == "ios"){
				IBeaconClass.addEventListener('ibeaconaccuracyupdate', function(arg){
					...
					if(isNewIBeacon(iBeaconID)){
                        var newibeacon = new BC.IBeacon({iBeaconID:iBeaconID,accuracy:arg.accuracy,proximity:arg.proximity,minorString:arg.minor});
						...
						theibeacon.dispatchEvent("ibeaconaccuracyupdate",theibeacon);
	...
	var StartIBeaconAdvertising = BC.IBeaconManager.StartIBeaconAdvertising = function(success,error,proximityUUID,major,minor,identifier){
		IBeaconClass.startIBeaconAdvertising(success,error,proximityUUID,major,minor,identifier);
	};
	...
	var StartIBeaconScan = BC.IBeaconManager.StartIBeaconScan = function(proximityUUID,major,minor){
		if(API.toLowerCase() == "ios"){
			IBeaconClass.startIBeaconScan(null,null,proximityUUID,major,minor);
		}else{
			var region = {};
	...
	var StopIBeaconScan = BC.IBeaconManager.StopIBeaconScan = function(proximityUUID,major,minor){
		if(API.toLowerCase() == "ios"){
			IBeaconClass.stopIBeaconScan(null,null,proximityUUID,major,minor);
		}else{
    ...
    /**
    * Storage RSSI And Metre.
    * @memberof IBeaconManager
    * @method
    * @example
    * BC.IBeaconManager.StorageRSSIAndMetre(metreValue,advCountValue);
    * @param {int} [metreValue] -
    * @param {int} [advCountValue] -
    */
    var StorageRSSIAndMetre = BC.IBeaconManager.StorageRSSIAndMetre = function(success,error,metreValue,advCountValue,beaconMinor){
        if(API.toLowerCase() == "ios"){
            IBeaconClass.storageRSSIAndMetre(success,error,metreValue,advCountValue,beaconMinor);
        }else{
            
        }
    };
	
   var SaveBeaconRSSIInfo = BC.IBeaconManager.SaveBeaconRSSIInfo = function(success,error){
        if(API.toLowerCase() == "ios"){
            IBeaconClass.saveBeaconRSSIInfo(success,error);
        }else{
